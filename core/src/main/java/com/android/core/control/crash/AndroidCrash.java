...
public class AndroidCrash implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(final Thread thread, final Throwable ex) {
        ...
        String time1 = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date(current));
        String time2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(current));
        final File file = new File(LogWriter.PATH + LogWriter.FILE_NAME + time1 + LogWriter.FILE_NAME_SUFFIX);

        //为了防止上传时文件没有写入完成
        LogWriter.writeLog(mContext, ex, file, time2, new LogWriter.WriteCallback() {
            @Override
